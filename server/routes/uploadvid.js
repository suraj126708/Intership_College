...
// MongoDB Connection
const mongoURI = "mongodb+srv://sumedhagitte6708:Suraj6708@expensetracker.7irfi.mongodb.net/InternShipCollege?retryWrites=true&w=majority&appName=expenseTracker";
...
router.post('/upload-interview', upload.single('video'), (req, res) => {
  ...
  const filename = `interview_${Date.now()}${path.extname(req.file.originalname)}`;
...
router.get('/list-interviews', async (req, res) => {
  try {
    ...
    res.status(500).json({ message: 'Error listing videos', error: error.message });
  }
});
